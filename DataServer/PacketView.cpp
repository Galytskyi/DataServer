...
QVariant PacketModel::data(const QModelIndex &index, int role) const
{
	...
	if (role == Qt::TextAlignmentRole)
	{
		...
		switch (columnIndex)
		{
			case PACKET_VIEW_COLUMN_IMEI:		result = Qt::AlignCenter;	break;
			case PACKET_VIEW_COLUMN_LOCATION:	result = Qt::AlignCenter;	break;
			case PACKET_VIEW_COLUMN_TIME:		result = Qt::AlignCenter;	break;
			case PACKET_VIEW_COLUMN_BRIGHT:		result = Qt::AlignCenter;	break;
			case PACKET_VIEW_COLUMN_SIZE:		result = Qt::AlignCenter;	break;
...
void PacketView::createHeaderContextMenu()
{
	...
	for(int column = 0; column < PACKET_VIEW_COLUMN_COUNT; column++)
	{
		QAction* pAction = m_headerContextMenu->addAction(qApp->translate("PacketView.h", PacketViewColumn[column]));
		if (pAction == nullptr)
		{
			continue;
		}
	...
	hideColumn1(PACKET_VIEW_COLUMN_BRIGHT, true);
	hideColumn1(PACKET_VIEW_COLUMN_SIZE, true);
}
