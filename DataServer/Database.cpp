...
int SqlFieldBase::init(int objectType, int)
{
	switch(objectType)
	{
		...
		case SQL_TABLE_DEVICE:

			append("ObjectID",						QMetaType::Int);
			append("DeviceID",						QMetaType::Int);
		...
		case SQL_TABLE_PICTURE_INFO:
			...
			append("ImageType",						QMetaType::Int);
			append("ImageSize",						QMetaType::Int);
...
int SqlTable::read(void* pRecord, int* key, int keyCount)
{
	...
	while (query.next() == true)
	{
		...
		switch(m_info.objectType())
		{
			...
			case SQL_TABLE_DEVICE:
				{
					DeviceParam* pDevice = static_cast<DeviceParam*> (pRecord) + readedCount;
					if (pDevice == nullptr)
					{
						break;
					}

					pDevice->setDeviceID(query.value(field++).toInt());
			...
			case SQL_TABLE_PICTURE_INFO:
				{
					...
					packet->setImei(query.value(field++).toULongLong());

					packet->setPacketTime(query.value(field++).toDateTime());

					packet->setImageType(query.value(field++).toInt());
					packet->setImageSize(query.value(field++).toInt());
					packet->setImageWidth(query.value(field++).toInt());
					packet->setImageHeight(query.value(field++).toInt());

					packet->setBrightness(query.value(field++).toInt());

					packet->setLocation(query.value(field++).toString());
				}
				break;
...
int SqlTable::write(void* pRecord, int count, int* key)
{
	...
	for (int r = 0; r < count; r++)
	{
		...
		switch(m_info.objectType())
		{
			...
			case SQL_TABLE_DEVICE:
				{
					DeviceParam* pDevice = static_cast<DeviceParam*> (pRecord) + r;
					if (pDevice == nullptr)
					{
						break;
					}
					...
					if (key == nullptr)
					{
						// for append record
						//
						pDevice->setDeviceID(lastKey() + 1);
					}

					query.bindValue(field++, pDevice->deviceID());
			...
			case SQL_TABLE_PICTURE_INFO:
				{
					...
					query.bindValue(field++, packet->imei());

					query.bindValue(field++, packet->packetTime());

					query.bindValue(field++, packet->imageType());
					query.bindValue(field++, packet->imageSize());
					query.bindValue(field++, packet->imageWidth());
					query.bindValue(field++, packet->imageHeight());

					query.bindValue(field++, packet->brightness());

					query.bindValue(field++, packet->location());
				}
				break;
...
bool Database::open()
{
	...
	//m_databaseOption.setType(OT::DatabaseType::PostgreSQL);
